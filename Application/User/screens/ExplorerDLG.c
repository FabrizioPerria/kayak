...
#define ID_UPDIRECTORY_BUTTON  	(GUI_ID_USER + 0x04)
#define ID_BACK_BUTTON  		(GUI_ID_USER + 0x05)
#define ID_TEXT_CMD 	 		(GUI_ID_USER + 0x06)
#define ID_TEXT_PATH 	 		(GUI_ID_USER + 0x07)
#define ID_LOGO_KAYAK 	 		(GUI_ID_USER + 0x08)
...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
	...
	{ TEXT_CreateIndirect, "Path", ID_TEXT_PATH, 150, 420, 500, 40, 0, 0x0, 0 },
};
...
static void loadFileList(WM_HWIN *hItem, const char* path){
	...
	while(StorageGetFileInfo(&fileInfo) != FR_NO_FILE){
		char *fields[4] = {fileInfo.name, fileInfo.ext, fileInfo.size, fileInfo.attrib};
	...
	case WM_NOTIFY_PARENT:
		...
		case ID_FILELIST_LISTVIEW:
			...
			case WM_NOTIFICATION_SEL_CHANGED:
				...
				hItem = WM_GetDialogItem(pMsg->hWin, ID_SELECTFILE_BUTTON);
				if(!isFile[0]){
...
#ifdef _USE_BITMAP_PICTURES
					BUTTON_SetBitmap(pMsg->hWinSrc, 0, &bmselectRelease);
#endif
					hListView = WM_GetDialogItem(pMsg->hWin, ID_FILELIST_LISTVIEW);
